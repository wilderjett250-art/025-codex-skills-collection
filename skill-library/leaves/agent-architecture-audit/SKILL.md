...
description: Audit or debug an agent/LLM application for wrapper regression, memory pollution, tool failures, repeated calls, loops, timeouts, state errors, hidden repair passes, or rendering corruption.
metadata:
...
**Do not use for:**
- General application code debugging — use a language or framework specialist.
...
- For one-run loops, timeouts, repeated tool calls, or state errors, read `~/.codex/skill-library/leaves/agent-introspection-debugging/SKILL.md` as the runtime-recovery branch.
- `agent-evaluation` — Benchmark agent performance and validate regressions
