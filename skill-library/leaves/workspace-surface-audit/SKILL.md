...
description: 'Audit a repository or Codex installation for MCPs, plugins, connectors, automations, hooks, prompts, permissions, Skill routing, environment state, secrets exposure, and harness risk.'
metadata:
  origin: ECC
---
...
Load only the branch required by the request:

- Automation live-state audit: `~/.codex/skill-library/leaves/automation-audit-ops/SKILL.md`
- Repository daily/library Skill classification: `~/.codex/skill-library/leaves/agent-sort/SKILL.md`
- Codex harness security, including prompts, MCPs, hooks, permissions, secrets, and package execution: `~/.codex/skill-library/leaves/security-scan/SKILL.md`. Adapt its Claude-specific paths to `.codex` and do not run external scanners or package installs without task authorization.
- Historical ECC configurator: `~/.codex/skill-library/leaves/configure-ecc/SKILL.md`. Use only as reference; current installation belongs to the system `skill-installer`.
