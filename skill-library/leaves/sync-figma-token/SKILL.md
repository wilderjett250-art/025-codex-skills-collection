...
- `<task-temp>/sync-figma-token-dry-run-{runId}.json`
- `<task-temp>/sync-figma-token-final-{runId}.json`

On Windows, resolve `<task-temp>` to `$env:TEMP`; on POSIX systems use
`${TMPDIR:-/tmp}`. The report is local diagnostic output and must not contain
