...
### Grey-first hierarchy for schematics and architecture figures

For model architectures, workflows, mechanism diagrams, and graphical method overviews, begin with tonal hierarchy rather than saturated category colors:

```python
PALETTE_GREY_FIRST = {
    "text_primary":   "#25292C",  # near-black charcoal, not pure black
    "structure_main": "#4A5054",  # primary data path and major outlines
    "structure_mid":  "#747B80",  # secondary branches and internal operators
    "structure_light":"#AEB4B8",  # scaffolds, guides, inactive states
    "fill_soft":      "#E7EAEC",  # restrained tensor/module fill
    "fill_faint":     "#F4F5F6",  # background grouping when genuinely useful
    "accent_muted":   "#6F8798",  # optional single semantic accent
}
```

Rules:

1. Use grey levels to encode hierarchy before introducing hue.
2. Reserve pure black for a very small number of critical marks, if it is needed at all.
3. Keep most explanatory text in near-black charcoal; use medium grey only for subordinate annotations, never for critical small text.
4. Use the muted accent on one semantically meaningful path or mechanism, not as decorative colored typography.
5. Avoid gradients, shadows, glossy fills, and repeated rounded cards; alignment, line weight, whitespace, and tonal contrast should carry the composition.
6. If the user asks for "mostly black text," interpret that as dark neutral typography, not an all-black figure.

This grey-first hierarchy is the default for schematics. Quantitative plots may instead use the semantic and NMI pastel palettes below when color encodes methods, groups, or signed effects.
