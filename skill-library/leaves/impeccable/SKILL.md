---
name: impeccable
description: Use to audit, refine, or harden an existing frontend, including critique, polish, accessibility, UX copy, and production readiness while preserving its identity unless redesign is requested.
metadata:
  version: 4.0.3
---
...
For concrete micro-polish involving nested radii, optical alignment, text wrapping, tabular numbers, image outlines, transition scope, or touch target size, load [reference/micro-polish.md](reference/micro-polish.md) together with the owning `polish`, `layout`, or `typeset` playbook. Do not load it for unrelated design work.
