...
This is a focused operator workflow for an explicitly located `ECC-Tools` repo. Resolve its live workspace path before acting; this is not a generic billing skill or a repo-wide code review pass.
...
- `research-ops` existing-solutions branch before inventing helpers or re-implementing repo-local utilities
- `security-review` when auth, usage gates, entitlements, or secrets are touched
- the verification branch in `terminal-ops` for proving rerun safety and exact post-fix state
