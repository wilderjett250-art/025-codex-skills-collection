...
| `django-engineering` | Django architecture, REST API with DRF, ORM, caching, signals, middleware |
| `django-engineering` | Django security: auth, CSRF, SQL injection, XSS prevention |
| `django-engineering` | Django testing with pytest-django, factory_boy, mocking, coverage |
| `django-engineering` | Django verification loop: migrations, linting, tests, security scans |
| `laravel-engineering` | Laravel architecture patterns: routing, controllers, Eloquent, queues, caching |
| `laravel-engineering` | Laravel security: auth, policies, CSRF, mass assignment, rate limiting |
| `laravel-engineering` | Laravel testing with PHPUnit and Pest, factories, fakes, coverage |
| `laravel-engineering` | Laravel verification: linting, static analysis, tests, security scans |
| `react-engineering` | React, Next.js, state management, performance, UI patterns |
| `frontend-slides` | Zero-dependency HTML presentations, style previews, and PPTX-to-web conversion |
| `go-engineering` | Idiomatic Go patterns, conventions for robust Go applications |
| `go-engineering` | Go testing: table-driven tests, subtests, benchmarks, fuzzing |
| `java-coding-standards` | Java coding standards for Spring Boot and Quarkus: naming, immutability, Optional, streams, CDI |
| `python-engineering` | Pythonic idioms, PEP 8, type hints, best practices |
| `python-engineering` | Python testing with pytest, TDD, fixtures, mocking, parametrization |
| `quarkus-engineering` | Quarkus architecture, Camel messaging, CDI services, Panache data access |
| `quarkus-engineering` | Quarkus security: JWT/OIDC, RBAC, input validation, secrets management |
| `quarkus-engineering` | Quarkus TDD with JUnit 5, Mockito, REST Assured, Camel testing |
| `quarkus-engineering` | Quarkus verification: build, static analysis, tests, native compilation |
| `springboot-engineering` | Spring Boot architecture, REST API, layered services, caching, async |
| `springboot-engineering` | Spring Security: authn/authz, validation, CSRF, secrets, rate limiting |
| `springboot-engineering` | Spring Boot TDD with JUnit 5, Mockito, MockMvc, Testcontainers |
| `springboot-engineering` | Spring Boot verification: build, static analysis, tests, security scans |
...
| `agent-evaluation` | Formal evaluation and AI-regression framework |
...
| `terminal-ops` | Evidence-first execution with a comprehensive verification branch |
...
| Skill | Description |
|-------|-------------|
| `research-ops` | Current research router with existing-solution discovery and multi-source synthesis branches |
...
| Skill | Description |
|-------|-------------|
| `agent-orchestration` | Multi-agent routing, including the dmux branch when explicitly selected |
...
- `django-engineering` may reference `django-engineering`
- `laravel-engineering` may reference `laravel-engineering`
- `quarkus-engineering` may reference `quarkus-engineering`
- `springboot-engineering` may reference `springboot-engineering`
- `continuous-learning-v2` references `~/.claude/homunculus/` directory
- `python-engineering` may reference `python-engineering`
- `go-engineering` may reference `go-engineering`
- `crosspost` references `content-engine` and `x-api`
- `research-ops` references `exa-search` (complementary MCP tools)
...
3. **Issue**: What's wrong (e.g., "references ~/.claude/skills/python-engineering but python-engineering was not installed")
4. **Suggested fix**: What to do (e.g., "install python-engineering skill" or "update path to .claude/skills/")
