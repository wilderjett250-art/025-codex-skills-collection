...
description: Use to write, review, migrate, or optimize YARA-X malware-detection rules, including naming, string selection, performance, modules, threat-hunting context, and false-positive reduction.
---
...
See the [testing guide](references/testing.md) for the rule-validation workflow; run each bundled script with `--help` for its current command options.
