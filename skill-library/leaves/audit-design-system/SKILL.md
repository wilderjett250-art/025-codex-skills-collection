...
description: Audit, apply, or repair Figma design-system integration. Use for missing shared components, local overrides, unbound tokens, one narrow finding, or coordinated reconciliation across multiple sections.
---
...
6. When the user wants a fix, route to the right downstream skill.
   For one specific offending node, use the fix-one branch at `~/.codex/skill-library/leaves/fix-design-system-finding/SKILL.md`.
   For a broader screen-wide pass or coordinated remediation across multiple sections, use the apply branch at `~/.codex/skill-library/leaves/apply-design-system/SKILL.md`.
...
- one concrete finding with a narrow write scope: read `~/.codex/skill-library/leaves/fix-design-system-finding/SKILL.md` and use its fix-one mode
- several findings that collapse into a broader screen or section reconciliation pass: read `~/.codex/skill-library/leaves/apply-design-system/SKILL.md` and use its apply mode
