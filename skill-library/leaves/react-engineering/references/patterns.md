...
The preserved hook rules are summarized below; this reference no longer depends on a separate `rules/react` bundle.
...
- Run `axe` in component tests (see [skills/react-engineering](../SKILL.md))
- For broader WCAG criteria, use the repository's accessibility guidance or the dedicated accessibility tooling available in the current environment.
...
- Router: [react-engineering](../SKILL.md) selects patterns, performance, and testing references.
- Framework comparison: [angular-developer](../../angular-developer/SKILL.md).
